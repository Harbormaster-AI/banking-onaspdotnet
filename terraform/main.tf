...
provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      CertificationId = "712664c3-ab54-4454-9157-6c52dc5b1c0b"
    }
  }
}
...
resource "aws_key_pair" "generated" {
  key_name   = "pjsk-sshtest-0.11469174158415374"
...
resource "aws_db_subnet_group" "default" {
  name       = "bankingonaspdotnet-db-subnet"
...
#  name        = "bankingonaspdotnet-security-group-from-terraform" #optional, when omitted, terraform creates a random name
  description = "security group for application bankingonaspdotnet created from terraform"
  vpc_id      = aws_vpc.default.id
...
resource "aws_security_group" "db" {
  description = "security group for bankingonaspdotnet and mysql created from terraform"
  vpc_id      = aws_vpc.default.id
...
#  identifier             = "bankingonaspdotnet-rds" # Terraform will create a unique id if not assigned
  allocated_storage      = 20
  engine                 = "mysql"
  instance_class         = "db.t3.medium"
  db_name                = "bankingonaspdotnet"
...
resource "aws_eks_cluster" "this" {
  name     = "eks_cluster_bankingonaspdotnet"
...
resource "aws_instance" "web" {
  ...
  tags = { Name = "bankingonaspdotnet instance" } 
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker pull theharbormaster/banking-onaspdotnet:latest",
      "sudo docker run -d -p 8000:8000 -p 8080:8080 -e DATABASE_URL=jdbc:mysql://${aws_db_instance.default.endpoint}/bankingonaspdotnet theharbormaster/banking-onaspdotnet:latest"
    ]
  }
}
